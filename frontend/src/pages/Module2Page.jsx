...
import { brand, so2Content, saveFormLocally } from "../mockData";
...
export default function Module1Page() {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      await axios.post(`${API}/so2-requests`, {
        ...data,
        source_page: "so2",
      });
      ...
      const ok = saveFormLocally("osh-so2-requests", data);
    ...
    <div style={{ fontFamily: brand.fonts.body }}>
      ...
      <section className="relative">
        ...
        <div className="mx-auto max-w-screen-xl px-4 pt-20 pb-16 text-white">
          <h1 className="text-4xl md:text-5xl font-bold" style={{ fontFamily: brand.fonts.heading }}>{so2Content.hero.title}</h1>
          <p className="mt-3 text-white/90 max-w-2xl">{so2Content.hero.subtitle}</p>
          <Button className="mt-6" style={{ backgroundColor: brand.colors.gold, color: brand.colors.ink }}>{so2Content.hero.cta}</Button>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-screen-xl px-4 py-12">
        <Card>
          <CardContent className="pt-6">
            <p className="text-sm leading-relaxed">{so2Content.overview}</p>
          </CardContent>
        </Card>
      </section>
      ...
      <section className="mx-auto max-w-screen-xl px-4 py-4">
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
          {so2Content.keyTopics.map((topic) => (
      ...
      <section className="mx-auto max-w-screen-xl px-4 py-10">
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          {so2Content.highlights.map((point, idx) => (
      ...
      <section className="mx-auto max-w-screen-xl px-4 py-10">
        ...
        <Accordion type="single" collapsible className="w-full">
          {so2Content.faq.map((f, idx) => (
            <AccordionItem key={idx} value={`so2-${idx}`}>
